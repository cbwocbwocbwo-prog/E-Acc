...
def validate_receipt_images(
    ...
    # Windows OCR의 첫 판독으로 수행해 미세한 숫자 획이 다른 전처리와 섞여 사라지는
    # 현상을 막는다. 아래의 기본 OCR 결과와 합쳐 최종 판정한다.
    ...
    for angle in (90, 180, 270):
        ...
        if text.strip():
            candidates.append((angle, evaluate_ocr_text(transaction, text)))
    if not candidates:
    ...
    enhancement_angles = (0, 90, 180, 270)
    enhanced_candidates: list[tuple[int, ReceiptValidationResult]] = []
    for angle in enhancement_angles:
        try:
            enhanced_text = recognize_images(image_paths, angle, enhanced=True)
        except (OSError, ValueError):
            enhanced_text = ""
        if not enhanced_text.strip():
            continue
        base = base_by_angle.get(angle)
        merged = evaluate_ocr_text(
            transaction,
            "\n".join(filter(None, (base.ocr_text if base else "", enhanced_text))),
        )
